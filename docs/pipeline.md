...
| **Render** | FFmpeg produces longform (16:9) and/or shortform (9:16) MP4s plus a JPEG thumbnail per clip (which formats run is controlled under **Settings → Pipeline** and can be overridden per run on the run detail page) | `rendered/longform/*.mp4`, `rendered/longform/*.jpg`, `rendered/shortform/*.mp4`, `rendered/shortform/*.jpg` |
...
**API:** `POST /api/runs/{id}/start` accepts `skip_llm_plan: true` to use the heuristic planner; without it, the API returns **400** when the LLM is not configured. The same request accepts `subtitles_disabled: true` to skip burned-in subtitles for that start when global subtitles are enabled (see **[configuration.md — Subtitles](configuration.md#subtitles-sqlite-settings)**). It also accepts `produce_longform` and `produce_shortform` (booleans, default `true`) to choose which aspect ratios to plan and render for that start; both cannot be `false` (at least one must stay enabled). The **web UI** loads defaults from **Settings → Pipeline** and sends explicit booleans on each start. The worker resolves **Settings** when a run has no override in `extra` (e.g. older tooling).
