...
When **main** is unset or `auto`, the order tried is: tavily → brave → exa → firecrawl → gemini → grok → kimi → minimax → ollama_web → perplexity → searxng → **duckduckgo**. The first provider that is “configured” wins: API keys unlock the paid/free-API providers; **DuckDuckGo is always available** (no key) and is the default when no other provider is configured.
...
| `[duckduckgo.py](duckduckgo.py)` | `duckduckgo` | *(none)*                                                                       | Unofficial HTML search via `duckduckgo-search` (DDGS), same class of integration as [OpenClaw’s DuckDuckGo tool](https://docs.openclaw.ai/tools/duckduckgo-search). Optional: `DUCKDUCKGO_REGION`, `DUCKDUCKGO_SAFE_SEARCH`, `DUCKDUCKGO_TEXT_BACKEND`, timeouts. |
